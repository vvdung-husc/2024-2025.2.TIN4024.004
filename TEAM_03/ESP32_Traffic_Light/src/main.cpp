...
void loop() {
  ...
  if (digitalRead(btn1) == LOW) {  // Kiểm tra nút nhấn để tắt hiển thị
    delay(50);  // Debounce
    if (digitalRead(btn1) == LOW) {
      led1State = !led1State;  // Đảo trạng thái LED1
      digitalWrite(led1, led1State ? HIGH : LOW);  // Bật/Tắt LED1
      if (!led1State) {  // Nếu LED1 tắt, xóa màn hình
      ...
      while (digitalRead(btn1) == LOW);  // Chờ thả nút
    }
  }
}
...
bool IsReady(ulong &ulTimer, uint32_t milisecond) {
  if (currentMillis - ulTimer < milisecond) return false;  // Kiểm tra xem đã đủ thời gian chưa
...
void NonBlocking_Traffic_Light() {
  switch (currentLED) {
    case rLED:  // Đèn đỏ
      if (IsReady(ledTimeStart, rTIME)) {  // Kiểm tra thời gian đèn đỏ
    ...
    case gLED:  // Đèn xanh
      if (IsReady(ledTimeStart, gTIME)) {  // Kiểm tra thời gian đèn xanh
        digitalWrite(gLED, LOW);
    ...
    case yLED:  // Đèn vàng
      if (IsReady(ledTimeStart, yTIME)) {  // Kiểm tra thời gian đèn vàng
        digitalWrite(yLED, LOW);
        digitalWrite(rLED, HIGH);
        currentLED = rLED;
