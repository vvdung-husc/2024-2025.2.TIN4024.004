...
#define BLYNK_TEMPLATE_ID "TMPL6-INQARE-"
#define BLYNK_TEMPLATE_NAME "ESP32API"
#define BLYNK_AUTH_TOKEN "YGrP3TRpLUPY02AQdMEfT6p-vv98SdeZ"
...
#define OPENWEATHERMAP_KEY "69283e6dafb70cdfac04a52aeda5043d"
...
void getAPI()
{
  ...
  if (httpResponseCode > 0)
  {
    ...
    Serial.println(urlGoogleMaps.c_str());
...
void updateTemp()
{
  ...
  if (!IsReady(lastTime, 10000))
    return; // Kiểm tra và cập nhật lastTime sau mỗi 100 giây
  if (WiFi.status() != WL_CONNECTED)
  {
  ...
  if (httpResponseCode > 0)
  {
    ...
    else
    {
      ...
      if (temp_ != temp)
      { // có thay đổi mới cập nhật lên Blynk
        temp_ = temp;
        Serial.print("Nhiệt độ: ");
        Serial.println(temp);
        Blynk.virtualWrite(V0, temp_);
      }
    }
  }
  else
...
void updateHumi()
{
  ...
  if (httpResponseCode > 0)
  {
    ...
    {
      float humidity = doc["main"]["humidity"]; // lấy thông tin nhiệt độ
...
void onceCalled()
{
  ...
  Blynk.virtualWrite(V3, link.c_str());        // Gửi giá trị lên chân ảo V3 trên ứng dụng Blynk (Map).
}
...
void uptimeBlynk()
{
  ...
  Blynk.virtualWrite(V2, value); // Gửi giá trị lên chân ảo V2 trên ứng dụng Blynk.
}
...
void loop(void)
{
  // return; // comment để chạy vòng lặp
