// Thông tin Blynk
#define BLYNK_TEMPLATE_ID "TMPL6aMXsufU8"
#define BLYNK_TEMPLATE_NAME "ESP8266 Blynk"
#define BLYNK_AUTH_TOKEN "MAsFLsyTMSTSZopEPse4r_fWMh99Zb4T"
...
// Thông tin WiFi
#define WIFI_SSID "LAB405-36 0267"
#define WIFI_PASS "223344556677"
...
void updateDHT() {
  ...
  oled.setCursor(0, 20);
  oled.printf("Nhiệt độ: %.2f°C", fTemperature);
  oled.setCursor(0, 40);
  oled.printf("Độ ẩm: %.2f%%", fHumidity);
  oled.sendBuffer();

  // Gửi dữ liệu lên Blynk
  Blynk.virtualWrite(V2, fTemperature);
  Blynk.virtualWrite(V3, fHumidity);
}

// Gửi thời gian chạy lên Blynk
void sendUptimeToBlynk() {
  Blynk.virtualWrite(V1, (int)((millis() - startTime) / 1000));
}
