...
bool displayState = true; // Trạng thái hiển thị màn hình (true: hiển thị, false: tắt)
...
void setup()
{
  ...
  pinMode(bPushButton, INPUT_PULLUP);  // Kéo lên nội bộ để tránh tín hiệu nhiễu
  pinMode(bLED, OUTPUT);         // Cài đặt chân LED xanh dương là OUTPUT
...
void toggleDisplay()
{
  bool buttonState = digitalRead(bPushButton);

  if (buttonState == LOW && lastButtonState == HIGH) // Phát hiện nhấn nút
  {
    displayState = !displayState; // Đảo trạng thái hiển thị màn hình
    ...
    if (displayState)
    {
      ...
      digitalWrite(bLED, HIGH); // Bật LED xanh dương
    }
    else
    {
      display.clear(); // Tắt màn hình
      digitalWrite(bLED, LOW); // Tắt LED xanh dương
    }
    delay(200); // Tránh lỗi nhấn giữ (debounce)
  }
